...
class FunctionalTestLocalStorageService implements LocalStorageService {

	private static ConcurrentMap<String, DataStore> stores = new ConcurrentHashMap<>();
	...
	@Override
	public DataStore getDataStore(String s) {
		return createDataStore(s);
	}

	public static DataStore createDataStore(String s) {
		if (stores.containsKey(s)) {
			return stores.get(s);
		}

		DataStore newStore = new FunctionalTestDataStore();
		stores.put(s, newStore);
		return newStore;
	}

	public static class FunctionalTestDataStore implements DataStore {
