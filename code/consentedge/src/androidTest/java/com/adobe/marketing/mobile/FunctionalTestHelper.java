...
public class FunctionalTestHelper {
		...
		public Statement apply(final Statement base, final Description description) {
			return new Statement() {
				...
				public void evaluate() throws Throwable {
					...
					MobileCore.setPlatformServices(new CorePlatformServices());
					MobileCore.setLogLevel(LoggingMode.VERBOSE);
					MobileCore.setApplication(application);
					FunctionalTestLocalStorageService.clearStores();
					...
					localStorageService.getDataStore("EdgeDataStorage").removeAll();
	...
	private static String collectLogCat(final String methodName) {
		...
		try {
			// TODO find better way to dump logs
	...
	public static void assertExpectedEvents(final boolean ignoreUnexpectedEvents) throws InterruptedException {
		...
		for (Map.Entry<EventSpec, ADBCountDownLatch> expected : expectedEvents.entrySet()) {
			boolean awaitResult = expected.getValue().await(FunctionalTestConstants.Defaults.WAIT_EVENT_TIMEOUT_MS,
